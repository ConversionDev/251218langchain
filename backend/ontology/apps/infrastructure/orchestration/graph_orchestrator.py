...
from infrastructure.orchestration.states.langgraph_state import ChatState
...
def _supports_tool_calling(provider: Optional[str] = None) -> bool:
    ...
    from infrastructure.llm.exaone_provider import supports_tool_calling  # type: ignore
...
def _get_llm_provider():
    """LLMProvider 클래스 반환."""
    from infrastructure.llm.exaone_provider import LLMProvider  # type: ignore
    ...
    if images:
        ...
        for msg in reversed(messages):
            if isinstance(msg, HumanMessage):
                ...
                break
        from infrastructure.llm.gemini_adapter import generate_multimodal  # type: ignore
