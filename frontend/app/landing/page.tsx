    ...
    <div className="font-sans relative min-h-screen overflow-hidden bg-[#f5f5f5]">
      <section className="relative h-[72vh] min-h-[420px] max-h-[720px] w-full overflow-hidden flex flex-col">
        <header className="absolute top-0 left-0 right-0 z-20 flex min-h-[4.5rem] items-center justify-between border-b border-[#a8d5c4]/50 bg-white/85 px-6 py-3 backdrop-blur-md dark:border-primary/20 dark:bg-[#0f0f0f]/90 md:px-8 md:py-4">
          <div className="flex flex-1 items-center gap-8 md:gap-10">
            <Link href="/landing" className="flex flex-col leading-tight shrink-0">
          ...
          <div className="flex shrink-0 items-center gap-3">
            <Link href="/demo" className="text-sm font-semibold text-slate-700 hover:text-slate-900 transition-colors px-3 py-2 md:text-base">
              데모
            </Link>
            <Link href="/login" className="rounded-md bg-slate-700 px-4 py-2 text-sm font-semibold text-white transition-colors hover:bg-slate-800 md:text-base">
              로그인
            </Link>
          </div>
        </header>
        <div
          ...
          style={{
            backgroundImage: `repeating-linear-gradient(-45deg, transparent, transparent 12px, rgba(220, 38, 38, 0.07) 12px, rgba(220, 38, 38, 0.07) 14px), repeating-linear-gradient(45deg, transparent, transparent 12px, rgba(220, 38, 38, 0.05) 12px, rgba(220, 38, 38, 0.05) 14px)`,
        ...
        <div className="absolute inset-0 z-[1]">
          {HERO_BG_IMAGES.map((src, i) => (
            ...
            >
              ...
              <div className="absolute inset-0 bg-gradient-to-l from-white/85 via-white/30 to-transparent" aria-hidden />
            </div>
          ))}
        </div>
          ...
          <div className="flex justify-center gap-3">
            {HERO_BG_IMAGES.map((_, i) => (
              <button
                ...
                style={{ backgroundColor: i === heroSlide ? "#e8f5ef" : "transparent" }}
                aria-label={`${i + 1}번째 슬라이드`}
                aria-current={i === heroSlide ? "true" : undefined}
              />
            ))}
          </div>
          <a href={`#${FEATURES_ID}`} className="flex flex-col items-center gap-1 text-sm font-medium text-slate-600 transition-colors hover:text-slate-900" aria-label="아래 섹션으로 스크롤">
            <span>더 보기</span>
        ...
        <div className="mx-auto max-w-6xl px-6 py-16">
          ...
          <div className="mt-12 grid gap-8 md:grid-cols-3">
            <div className="rounded-2xl border border-slate-200 bg-white p-8 shadow-sm transition-shadow hover:shadow-md">
              ...
              <h3 className="mt-4 text-lg font-semibold leading-tight tracking-tight text-slate-900">정확한 인재 매칭</h3>
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-8 shadow-sm transition-shadow hover:shadow-md">
              ...
              <h3 className="mt-4 text-lg font-semibold leading-tight tracking-tight text-slate-900">간편한 공시 리포팅</h3>
              <p className="mt-3 text-[15px] leading-[1.65] text-slate-600">
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-8 shadow-sm transition-shadow hover:shadow-md">
              ...
              <h3 className="mt-4 text-lg font-semibold leading-tight tracking-tight text-slate-900">데이터 기반 인사이트</h3>
              <p className="mt-3 text-[15px] leading-[1.65] text-slate-600">
      ...
      <footer className="relative z-10 border-t border-slate-200 bg-white py-8">
        <div className="mx-auto max-w-6xl px-6 text-center">
          <p className="text-[15px] text-slate-600">© 2026 HRInsight. 차세대 인적자본 관리 시스템</p>
        </div>
      </footer>
    </div>
  );
