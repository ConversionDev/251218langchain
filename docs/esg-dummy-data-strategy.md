...
> **손실량(scrap)·산출 폐기물(waste)** 도 **만드는 김에 같이 생성** — 학습·효율 지표·추후 Rule 확장용.
...
| process | line | 설비 정격(kW) | 정상 usage 범위(kWh) | 정상 production | 정상 equipment_ct | day 범위 | night 범위 | usage/production | usage/equipment_ct | 정상 scrap | 정상 waste |
|---------|------|----------------|----------------------|-----------------|-------------------|----------|------------|------------------|---------------------|------------|------------|
| mixing  | L1   | 600            | 350~500              | 80~120          | 4~6               | 350~500  | 60~100     | 4.5~5.5          | 70~95               | 2~8        | 1~4        |
| plate   | L1   | 500            | 280~420              | 100~150         | 3~5               | 280~420  | 50~80      | 2.8~3.2          | 75~95               | 2~6        | 1~3        |
| cell    | L2   | 700            | 400~600              | 60~100          | 5~8               | 400~600  | 70~120     | 6.0~7.0          | 65~85               | 1~5        | 1~3        |
| module  | L1   | 550            | 300~450              | 40~70           | 3~5               | 300~450  | 40~70      | 6.5~7.5          | 75~95               | 1~4        | 0~2        |
| pack    | L1   | 450            | 250~380              | 30~50           | 2~4               | 250~380  | 30~60      | 7.5~8.5          | 90~110              | 0~3        | 0~2        |

- **설비 정격**: Rule 2 임계치.
- **scrap(손실량), waste(산출 폐기물)**: 학습·효율 지표용. 데이터셋 생성 시 **만드는 김에 같이 생성**. 단위는 production과 동일(EA 등). **임의 생성 시 통상 참고치**는 **`app/data/env_mapping/README_배터리_손실량_폐기물_통상참고치.md`** 에 정리됨(배터리 제조 논문·LCA에 자주 나오는 수율·손실률 기반). 표의 범위는 그 통상치로 맞추거나, 자사 MES 확보 후 갱신.
...
- **로직 요약**:
  ...
  - **`scrap`(손실량), `waste`(산출 폐기물)**: 프로파일의 정상 범위에서 랜덤. **만드는 김에 같이 생성** — 학습·효율 지표·추후 Rule 확장용.
...
## 4.5 measurements 확장 — 손실량·산출 폐기물

데이터셋(더미/학습용) 생성 시 **만드는 김에** 다음 컬럼을 포함해 둠.

| 컬럼 | 설명 | 단위 |
|------|------|------|
| **scrap** | 손실량(스크랩·불량 등) | production과 동일(EA 등) |
| **waste** | 산출 폐기물 | production과 동일(EA 등) |

- **용도**: 학습 피처·효율 지표(usage/(production+scrap), scrap rate 등). 추후 “손실/폐기물 이상” Rule 추가 시 기준으로 사용 가능.
- **정상 행**: 위 프로파일의 정상 scrap·waste 범위에서 샘플링. 해당 범위는 **배터리 관련** 참고로만 정함(배터리 자료 없으면 가설 범위 후 보강).
- **실무**: 실제 measurements는 MES에서 production과 함께 scrap·waste 수집 시 동일 컬럼으로 적재 가능.
