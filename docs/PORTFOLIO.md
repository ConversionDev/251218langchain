...
- **작업 특성별 모델 분기**: 학습 EXAONE(GPU)·llama.cpp(CPU 배포)·Gemini(빠른 분류·OCR)를 적재적소 배치 — **핵심 역량 판정은 자체 파인튜닝 모델, 빠른 추출·분류는 관리형 API**
...
| 채팅 RAG 에이전트(SSE·도구·멀티모달) | ✅ 구현·배포 | 배포 CPU EXAONE라 응답 지연(아래 한계) |
...
- **배포 채팅 지연**: EXAONE를 CPU(llama.cpp GGUF)로 돌려 응답이 느림(수십 초, SSE로 점진 출력). 학습 모델 쇼케이스 vs 속도 트레이드오프. 필요 시 채팅도 Gemini 분기 가능.
