...
from domain.v1.spokes.spam.repositories.policy_repository import (  # type: ignore
    PolicyRepository,
)
from domain.v1.hub.orchestrators.graph_tools import TOOL_MAP  # type: ignore
...
class PolicyService:
    ...
    def _analyze_with_exaone(
        ...
        """EXAONE 툴을 사용하여 이메일 분석.

        EXAONE은 툴로 감싸진 형태로 사용되며, 모든 판별/판독은 이 툴을 통해 수행됩니다.
        어댑터 관리도 툴 내부에서 처리됩니다.
        ...
        """
        try:
            # analyze_with_exaone 툴 가져오기
            analyze_tool = TOOL_MAP.get("analyze_with_exaone")
            if not analyze_tool:
                raise ValueError("analyze_with_exaone 툴을 찾을 수 없습니다.")

            # 정책 정보를 email_metadata에 포함
            enhanced_metadata = email_metadata.copy()
            if policies:
                # 정책 정보를 텍스트로 추가
                policy_text = "\n".join([
                    f"- {policy.get('content', '')}" for policy in policies
                ])
                enhanced_metadata["_policy_context"] = policy_text

            # EXAONE 툴 호출 (어댑터 관리도 툴 내부에서 처리)
            tool_result_json = analyze_tool.invoke(enhanced_metadata)
