...
# app 디렉터리: domain/v1/spokes/chat/services -> app
_APP_DIR = Path(__file__).resolve().parent.parent.parent.parent.parent.parent
...
def _get_adapter_dir() -> Optional[Path]:
    ...
    if not base.exists():
        return None
    # 1) 루트에 adapter_config.json 있으면 루트 사용
    if (base / "adapter_config.json").exists():
        return base
    # 2) checkpoint-* 폴더 중 숫자가 가장 큰 것(최신) 사용
