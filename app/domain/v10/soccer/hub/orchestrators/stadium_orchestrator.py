...
from domain.v10.soccer.models.states.stadium_state import StadiumProcessingState  # type: ignore
from domain.v10.soccer.spokes.services.stadium_service import StadiumService  # type: ignore

logger = logging.getLogger(__name__)


def _determine_strategy_node(state: StadiumProcessingState) -> Dict[str, Any]:
    """
    전략 판단 노드.

    Stadium은 Agent가 없으므로 항상 Rule 기반을 사용합니다.
    ...
    logger.info("[DetermineStrategyNode] Stadium은 항상 Rule 기반 사용")
...
def _rule_process_node(state: StadiumProcessingState) -> Dict[str, Any]:
    """
    Rule 기반 처리 노드.

    StadiumService를 사용하여 명확한 규칙에 따라 데이터를 처리합니다.
...
def _route_strategy(state: StadiumProcessingState) -> str:
    """
    전략 라우팅 함수.

    Stadium은 항상 Rule 기반만 사용합니다.
...
def build_stadium_graph():
    ...
    g = StateGraph(StadiumProcessingState)
    ...
    def route_after_validate(state: StadiumProcessingState) -> str:
        """Validate 후 라우팅 결정"""
        errors = state.get("errors", [])
        if errors:
    ...
    def route_after_save(state: StadiumProcessingState) -> str:
        """Save 후 라우팅 결정"""
...
def get_stadium_graph():
    """
    Stadium 데이터 처리 그래프 인스턴스 반환 (lazy loading).
...
class StadiumOrchestrator:
    ...
    def _log_preview_data(self, preview_data: List[Dict[str, Any]]) -> None:
        ...
        """
        self.logger.info("=" * 80)
        self.logger.info("[StadiumOrchestrator] 상위 5개 데이터 미리보기")
        self.logger.info("=" * 80)
    ...
    def process(
        ...
        **kwargs
        ...
        Returns:
            처리 결과 딕셔너리
        """
        self.logger.info(f"[StadiumOrchestrator] LangGraph 처리 시작: 전체 {len(data)}개, 미리보기 {len(preview_data)}개")
        ...
        try:
            ...
            if db_session is not None:
                try:
                    ...
                    db_session.rollback()
                    self.logger.error("[StadiumOrchestrator] commit 실패: %s", commit_err)
                    return {
                        "success": False,
            ...
            self.logger.info(f"[StadiumOrchestrator] 처리 완료: {final_result}")
            ...
            self.logger.error(f"[StadiumOrchestrator] 처리 실패: {str(e)}")
            import traceback
            traceback.print_exc()
