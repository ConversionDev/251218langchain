...
from domain.v10.shared.data_loader import load_schedules_hybrid  # type: ignore
...
async def upload_schedule_jsonl(
    ...
    """
    스케줄 JSONL 파일을 업로드하고 첫 5개 행을 미리보기한 후, 전체 데이터를 DB에 저장합니다.
    ...
    try:
        ...
        try:
            logger.info(f"[업로드 시작] schedules 파일: {file.filename}")

            # 하이브리드 방식으로 데이터 로드 (전체 데이터 저장)
            results = load_schedules_hybrid(
                jsonl_path=tmp_path,
                db=db,
            ...
            # 에러 발생 시 DB 롤백
            db.rollback()
            logger.error(f"[업로드 실패] schedules 파일 처리 중 오류: {str(load_error)}")
            raise HTTPException(
                status_code=500,
