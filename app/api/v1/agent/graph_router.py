...
async def agent_chat(request: AgentRequest):
    ...
    try:
        ...
        if "core.llm.providers.llm_provider" in sys.modules:
            llm_provider_module = sys.modules["core.llm.providers.llm_provider"]
        else:
            llm_provider_module = importlib.import_module(
                "core.llm.providers.llm_provider"
            )
        LLMProvider = llm_provider_module.LLMProvider
        ...
        # 에이전트 실행
        provider = request.provider or LLMProvider.get_provider_name()
        response = run_agent(
...
async def list_providers():
    ...
    if "core.llm.providers.llm_provider" in sys.modules:
        ...
        llm_provider_module = importlib.import_module("core.llm.providers.llm_provider")
    LLMProvider = llm_provider_module.LLMProvider

    current_provider = LLMProvider.get_provider_name()
    providers = []

    for name in LLMProvider.list_providers():
        providers.append(
            ProviderInfo(
                name=name,
                supports_tool_calling=LLMProvider.supports_tool_calling(name),
...
async def agent_health():
    ...
    try:
        ...
        supports_tools = LLMProvider.supports_tool_calling(provider)

        return {
            "status": "healthy",
            "current_provider": provider,
            "supports_tool_calling": supports_tools,
            "available_providers": LLMProvider.list_providers(),
