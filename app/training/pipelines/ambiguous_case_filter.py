...
def filter_ambiguous_cases(
    ...
    ambiguous_cases = []
    scores = []
    early_stop_threshold = max_samples * 2 if max_samples else None
...
def sample_diverse(
    ...
    remainder = n % 3
    sampled = []
    for i, (_, bin_cases) in enumerate(bins.items()):
        if bin_cases:
...
def filter_training_data(
    ...
    if adaptive_filtering:
        ...
        sample_metadata = [extract_email_metadata(item) for item in train_data[:sample_size]]
        sample_scores = []
        try:
        ...
        std_dev = np.std(np.array(sample_scores))
        mean_score = np.mean(sample_scores)
        print(f"  샘플 점수 분포: 평균={mean_score:.3f}, 표준편차={std_dev:.3f}")
        if std_dev < 0.1:
            min_score = max(0.1, mean_score - 0.3)
            max_score = min(0.9, mean_score + 0.3)
        elif std_dev < 0.15:
            min_score = max(0.2, mean_score - 0.25)
            max_score = min(0.8, mean_score + 0.25)
        print()

    print("[Step 2] Train 데이터 필터링 중...")
    filtered_train = filter_ambiguous_cases(
        train_data, llama_gate,
    ...
    print(f"[OK] 저장 완료: {filtered_train_path}, {filtered_val_path}")
    print("=" * 60)
    print("[OK] 데이터 필터링 완료!")
    print("=" * 60)
...
def main():
    ...
    parser.add_argument("--max_score", type=float, default=0.65)
