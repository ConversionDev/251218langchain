...
### 0) 한 번에 실행 (전략 전체: 원본 73개 + 배터리 12개 언어 → 보강 Excel)
```bash
...
- 옵션: `--no-pubchem`, `--no-keci`, `--stage 1|2|3`, `--delay 0.5`, `--pubchem-delay 0.2`, `--keci-service-key`
...
### 2) 전체 보강 (원본 + 12개 언어 + PubChem + KECI + ECHA)
```bash
...
- 옵션: `--no-pubchem`, `--no-keci`, `--no-echa`, `--pubchem-delay 0.2`
